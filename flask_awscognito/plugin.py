...
class AWSCognitoAuthentication:
    ...
    def token_service(self):
        if not hasattr(g, CONTEXT_KEY_TOKEN_SERVICE):
            token_service = self.token_service_factory(
                self.user_pool_id, self.user_pool_client_id, self.region
            )
            setattr(g, CONTEXT_KEY_TOKEN_SERVICE, token_service)
        return getattr(g, CONTEXT_KEY_TOKEN_SERVICE)

    @property
    def cognito_service(self):
        if not hasattr(g, CONTEXT_KEY_COGNITO_SERVICE):
            ...
            setattr(g, CONTEXT_KEY_COGNITO_SERVICE, cognito_service)
        return getattr(g, CONTEXT_KEY_COGNITO_SERVICE)
